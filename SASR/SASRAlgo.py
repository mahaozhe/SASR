...
class TSRS_SAC(SAC):
    def __init__(self, env, actor_class, critic_class, exp_name="sac-tsrs", seed=1, cuda=0, gamma=0.99,
                 ...
                 alpha_autotune=True, write_frequency=100, save_folder="./sac-tsrs/", reward_weight=0.5,
                 kde_bandwidth=0.2, kde_sample_burn_in=10000, rff_dim=None, drop_rate_s=0.1, drop_rate_f=0.1,
                 drop_rate_s_end=None, drop_rate_f_end=None, drop_rate_s_fraction=None, drop_rate_f_fraction=None,
                 sample_rate=0.01, reward_length_frac=None):
        super(TSRS_SAC, self).__init__(env, actor_class, critic_class, exp_name, seed, cuda, gamma, buffer_size,
        ...
        # + a buffer to store the trajectory of states
        self.S_buffer = []
        # self.S_buffer_numpy = np.array(self.S_buffer)
        self.S_buffer_tensor = torch.Tensor(self.S_buffer).to(self.device)
        self.F_buffer = []
        # self.F_buffer_numpy = np.array(self.F_buffer)
        self.F_buffer_tensor = torch.Tensor(self.F_buffer).to(self.device)

        # + new for TSRS: to initialize the KDE model
        # self.kde_model = KernelDensity(kernel='gaussian', bandwidth=kde_bandwidth)
        # self.rff_model = RBFSampler(gamma=1.0 / (2 * kde_bandwidth ** 2), n_components=100, random_state=seed)
        # self.kde_model = SGDClassifier(loss='log_loss', max_iter=1000, tol=1e-3, random_state=seed)

        self.kde_bandwidth = kde_bandwidth
        self.kde_sample_burn_in = kde_sample_burn_in
        self.obs_dim = env.observation_space.shape[0]

        # + new for TSRS: to use RFF to approximate the kernel function
        if rff_dim is None:
            self.rff = False
        else:
            self.rff = True
            self.rff_dim = rff_dim
            # self.rff_W = np.random.normal(0, 1 / kde_bandwidth, size=(rff_dim, self.obs_dim))
            # self.rff_b = np.random.uniform(0, 2 * np.pi, size=(rff_dim,))
        ...
        # + new for TSRS: for the drop rates
        self.drop_rate_s = drop_rate_s
        self.drop_rate_f = drop_rate_f

        if drop_rate_s_end is not None:
            self.drop_rate_s_dec = True
            assert drop_rate_s_end < drop_rate_s, "the end drop rate should be smaller than the start drop rate"
            self.drop_rate_s_end = drop_rate_s_end
            self.drop_rate_s_fraction = drop_rate_s_fraction
        else:
            self.drop_rate_s_dec = False

            # + for the success, we adaptively change the drop rate with reward_length_frac
            # self.reward_length_frac = reward_length_frac

        if drop_rate_f_end is not None:
            self.drop_rate_f_dec = True
            assert drop_rate_f_end < drop_rate_f, "the end drop rate should be smaller than the start drop rate"
            self.drop_rate_f_end = drop_rate_f_end
            self.drop_rate_f_fraction = drop_rate_f_fraction
        else:
            self.drop_rate_f_dec = False

        # + new for TSRS: use the sample rate to control the sample value
        self.sample_rate = sample_rate

    def update_S(self, trajectory, total_timesteps, global_step):
        if self.drop_rate_s_dec:
            duration = self.drop_rate_s_fraction * total_timesteps
            slope = (self.drop_rate_s_end - self.drop_rate_s) / duration
            drop_rate = slope * global_step + self.drop_rate_s if global_step <= duration else self.drop_rate_s_end
        else:
            drop_rate = self.drop_rate_s

        drop_interval = int(1 / drop_rate) + 1
        if drop_interval >= len(trajectory):
            return
        trajectory = trajectory[::drop_interval]

        self.S_buffer += trajectory
        # self.S_buffer_numpy = np.array(self.S_buffer)
        self.S_buffer_tensor = torch.Tensor(np.array(self.S_buffer)).to(self.device)

        self.writer.add_scalar("charts/S_trajectory_length", len(trajectory), global_step)
        self.writer.add_scalar("charts/S_buffer_size", len(self.S_buffer), global_step)

    def update_F(self, trajectory, total_timesteps, global_step):
        if self.drop_rate_f_dec:
            duration = self.drop_rate_f_fraction * total_timesteps
            slope = (self.drop_rate_f_end - self.drop_rate_f) / duration
            drop_rate = slope * global_step + self.drop_rate_f if global_step <= duration else self.drop_rate_f_end
        else:
            drop_rate = self.drop_rate_f

        # + here we don't consider the drop_rate_f == 0
        drop_interval = int(1 / drop_rate)
        if drop_interval >= len(trajectory):
            return
        trajectory = trajectory[::drop_interval]
        ...
        self.writer.add_scalar("charts/F_trajectory_length", len(trajectory), global_step)
        self.writer.add_scalar("charts/F_buffer_size", len(self.S_buffer), global_step)
    ...
    def KDE_RFF_sample(self, buffer, batch):
        ...
        # kde_estimates = np.maximum(kde_estimates, 0)
    ...
    def learn(self, total_timesteps=1000000, learning_starts=5000):
        ...
        # + new for TSRS: to record the trajectory
        trajectory = []
        ...
        for global_step in range(total_timesteps):
            ...
            # + new for TSRS: record the trajectory
            ...
            if reward > 0:
                ...
                self.update_S(trajectory, total_timesteps, global_step)
                trajectory = []
                reward_positive = False
            ...
            if not done:
                ...
                if reward_positive:
                    ...
                    self.update_F(trajectory, total_timesteps, global_step)
    ...
    def optimize(self, global_step):
        ...
        with torch.no_grad():
            ...
            # + new for TSRS: to calculate the reward
            ...
            self.writer.add_scalar("charts/average_S_density", torch.mean(density_values_S).item(), global_step)
            self.writer.add_scalar("charts/average_F_density", torch.mean(density_values_F).item(), global_step)

            # sample from the beta distribution
            # shaped_rewards = np.random.beta(density_values_S + 1, density_values_F + 1)
            # shaped_rewards = torch.from_numpy(shaped_rewards).to(self.device).float()
            shaped_rewards = Beta(density_values_S + 1, density_values_F + 1).sample()

            next_q_value = data.rewards.flatten() + self.reward_weight * shaped_rewards + (
